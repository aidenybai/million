...
You are probably here because you got an warning message in your console.
...
Due to restrictions with the compiler, you must assign the block to a variable. This is because the compiler needs to analyze the block to ensure that it is valid.

```jsx
export default block(App); // ❌ Wrong

const AppBlock = block(App); // ✅ Correct

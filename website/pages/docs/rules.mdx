...
**It simply won't work!** Exactly why you need to add the compiler to your bundler, like this:
...
/\*_ @type {import('@rspack/cli').Configuration} _/
module.exports = {
...
<Callout type="warning">
  This section highlights some of the possible errors you might encounter using
  blocks
</Callout>

There are a number of things that could have caused you to encounter errors.

**Here are some errors / warnings and how to resolve them**:

<Callout type="warning">
  Found unsupported argument for block. Make sure blocks consume the reference
...
<Callout type="warning">
  You cannot use multiple returns in blocks. There can only be one return
...
<Callout type="warning">
  Components will cause degraded performance. Ideally, you should use DOM
  elements instead
</Callout>
...
<Callout type="warning">
  Expected a block statement for the component function. Make sure you are using
  a function declaration or arrow function
</Callout>
...
<Callout type="warning">Spread attributes aren't supported.</Callout>
...
function MyCorrectComponent({ props }) {
  return (
    <div name={props.name} style={props.styles}>
...
Well, legacy React has it such that we can have classes be used to create components, however that is old syntax now, but, if you happen to be finding it hard to be getting the error above; it might be because you
aren't using function-components. To resolve this, all you have to do is to use Functional Components.

Goodluck and make sure to smile like you just won a Million dollars.

...
A virtual DOM is a methodology enabling creating user interfaces declaratively, while efficiently manipulating the DOM. Instead, you create a virtual representation of the DOM, and the virtual DOM library will take care of updating the actual DOM for you.

The virtual DOM is used by many popular libraries, including [React](https://react.dev) and [Preact](https://preactjs.com/). However, it suffers from [performance overhead](https://svelte.dev/blog/virtual-dom-is-pure-overhead), making it less efficient.

Below you can see the "diff" algorithm needing to check against every node in the tree in order to make one simple change.
...
> Interested in playing with a simple implementation of the block virtual DOM? Check out [`hundred`](https://github.com/aidenybai/hundred/blob/main/src/index.ts)

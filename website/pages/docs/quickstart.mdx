...
import { CombinedBlockExample } from '../../components/examples/combined-block';
...
function Lion() {
  return (
    <img src="https://millionjs.org/lion.svg" />
  );
}
...
Blocks are great for rendering large lists, data grids, and many other use cases. The reason is because it's use static analysis in order to identify and directly mutate dynamic nodes.

Click on either tab to compare (or read the blog post for an [in-depth explanation](http://millionjs.org/blog/virtual-dom)):
...
As illustrated above, using a block can allow us to capture potential performance gains. However, you should always use best judgement, as blocks are not a silver bullet. Here are some general guidelines to follow:

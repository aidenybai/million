...
[![Open in StackBlitz](https://developer.stackblitz.com/img/open_in_stackblitz.svg)](https://stackblitz.com/github/aidenybai/million)
...
`block(Component){:jsx}` must consume a stateless component with primitive props, and returns a Block component.

<Callout>
  Block components must be deterministic, meaning that they must always return the same JSX. However, they can use hooks and other React features, except passing in JSX into expression.
</Callout>

...
Supongamos que tienes acceso a los componentes preconstruidos `<Table />{:jsx}` y `<Input />{:jsx}` de tu falsa librería UI. Luego, puedes almacenar el número de filas que deseas mostrar en un hook `useState(){:jsx}`.
...
function App() {
  ...
  return (
    <div>
      <Input value={filas} setValue={setFilas} />
      <Table>// ...</Table>
    </div>
  );
}
...
¡Pero espera! Has creado un grid pero no tienes datos. Supongamos que puedes obtener un array de datos arbitrarios de una función llamada `buildData(filas){:jsx}`:

```jsx
const data = buildData(100);
// devuelve [{ adjetivo: '...', color: '...', sustantivo: '...' }, ... x100]
```
...
function App() {
  const [filas, setFilas] = useState(1);
  const data = buildData(filas);

  return (
    <div>
      <Input value={filas} setValue={setFilas} />
      <Table>
        {data.map(({ adjetivo, color, sustantivo }) => (
          <tr>
            <td>{adjetivo}</td>
            <td>{color}</td>
            <td>{sustantivo}</td>
...
<Box code={`
  import { useState } from 'react';
  import { Table, Input, muchosElementos } from './ui';
  import { buildData } from './data';
...
function Fila({ adjetivo, color, sustantivo }) {
  return (
    <tr>
      <td>{adjetivo}</td>
      <td>{color}</td>
...
const RowBlock = block(function Fila({ adjetivo, color, sustantivo }) {
  return (
    <tr>
      <td>{adjetivo}</td>
      <td>{color}</td>
...
```jsx
data.map(({ adjetivo, color, sustantivo }) => (
  <RowBlock adjetivo={adjetivo} color={color} sustantivo={sustantivo}>
));
```
...
<For each={data}>
  {({ adjetivos, color, sustantivos }) => (
    <RowBlock adjetivos={adjetivos} color={color} sustantivos={sustantivos} />
  )}
</For>;
```
...
function Fila({ adjetivos, color, sustantivos }) {
return (
<tr>
<td>{adjetivos}</td>
<td>{color}</td>

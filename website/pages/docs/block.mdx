...
## Configuring SVG Blocks

By default, blocks are rendered into `<slot>` elements, but unfortunately SVGs do not support slots. To render pure SVG elements, you must turn on the `svg` option.

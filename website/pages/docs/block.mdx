...
## Customizing wrapper type

By default, blocks are rendered into `<slot>` elements. You can customize the wrapper type by a `type` option to the second argument.

```jsx
const SVGPartBlock = block(
  function SVGPart() {
    return (
      <g>
        <rect />

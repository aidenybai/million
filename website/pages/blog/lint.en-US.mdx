...
During runtime, these inject handlers capture render, timing, and metadata information as you interact with your web app in dev mode. With this approach, we can get runtime profiling from running the app, and have access to the source code thanks to the JavaScript compiler!
...
After capturing the renders, we extended the compiler to asynchronously collect bundle, network, and state manager information without build time overhead. These real-time insights are then fed into the VSCode extension, where you can see the collected information and suggestions to fix it.
...
After running Lint++ on the notes-editor app, it labeled the following problems across 6 components:
...
We invited an engineer who's never seen the code base before to try speed up the notes-editor app. Using Million Lint, he 3x the performance of the main page after ~15 minutes of work.

<div className="pt-4">
<video src="/lint/comparison.mp4" controls />
</div>

Of course, notes-editor is a small app, and Million Lint wouldn't always find the best fix for every problem. We are working hard to expand its net – try and please let us know about your experience!

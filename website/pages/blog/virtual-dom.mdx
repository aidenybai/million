...
<a href="https://www.youtube.com/watch?v=2-fR2rrmw3I">
  <Image src="/back-in-block.webp" width={300} height={130} />
</a>
...
The virtual DOM was created to address performance issues caused by frequent manipulation of the real DOM. It is a lightweight, in-memory representation of the real DOM, which can be later used as refernce to update the actual web page.
...
With newer frameworks like Svelte, the virtual DOM isn't even used because of the performance overhead. Instead, Svelte uses a technique called "dirty checking" to determine what has changed. Fine-grained reactivity frameworks like Solid.js take this a step further by pinpointing exactly what has changed and updating only that part of the DOM.
...
Since Million.js takes a similar approach to Blockdom, we'll be using Million.js as an example for the rest of this article.
...
This step is responsible for extracting dynamic parts of the virtual DOM into a the Edit Mapping, which stores the "edits" (mappings) of the dynamic parts of the virtual DOM to the state.

<StaticAnalysisExample />

### Dirty Checking

After the Edit Mapping is created, the dirty checking step can begin. This step is responsible for determining what has changed in the state, and updating the DOM accordingly.
...
The JavaScript Framework Benchmark is a benchmark that tests the performance of frameworks by rendering a large table of rows and columns. The benchmark is designed to test the performance highly unrealistic tests (like adding/replacing 1000 rows), and is not necessarily representative of real world applications.
...
The block virtual DOM is also good for "stable" UI trees, or UI trees that don't change much. This is because the Edit Mapping is only created once, and shouldn't need to be recreated on every render.
...
One of the biggest mistakes begineers make is using the block virtual DOM everywhere. This is a bad idea, because the block virtual DOM is not a silver bullet, and is not always faster than the regular virtual DOM.
...
That said, I'm excited to see what the future holds for it. Are you too?

...
const BigContainer = styled.div`
    width: 100%;
    height: 100%;
    background: linear-gradient(to bottom right, #b3e0f2, #16a085);
`
 ...
 function Keyboard({ correctLetters, incorrectLetters, addGuessedLetter, disabled = false }: Keyboard) {
  ...
  useEffect(() => {
    const handleKeyPress = (event: KeyboardEvent) => {
      ...
      if (!disabled && KEYS.includes(key) && !correctLetters.includes(key) && !incorrectLetters.includes(key)) {

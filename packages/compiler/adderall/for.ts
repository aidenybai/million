...
import { collectImportedBindings, resolveImportSource } from './mod';
import {
  createDeopt,
  getValidSpecifiers,
  handleVisitorError,
...
import { getNamedImports } from './mod';
...
export const transformFor = (
  ...
  const { namedImports, localImports } = getNamedImports(
    programPath,
    'million',
  );

  const jsxId = jsxElement.openingElement.name;
  if (!t.isJSXIdentifier(jsxId) || !localImports[jsxId.name]) {
    return;
  }
  ...
  const block = t.identifier(namedImports.block ?? 'block');
  if (!namedImports.block) {
    importDeclaration.specifiers.push(t.importSpecifier(block, block));
  }

  if (namedImports.For) return;

  trimJsxChildren(jsxElement);

  if (jsxElement.children.length !== 1) {
    throw createDeopt('For component must have exactly one child', file, path);
  }

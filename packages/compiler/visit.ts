...
export function babel(options: Options, filename: string): PluginObj {
  ...
  return {
    name: 'million',
    pre() {
      this.cache = new Map<string, t.Identifier>();
    },
    visitor: {
      ...
      CallExpression(path) {
        catchError(() => {
          transformBlock(options, path, this.cache as any, filename, info!);
        }, options.log);
      },

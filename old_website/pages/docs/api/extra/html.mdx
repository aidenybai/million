...
The `html` function creates a VNode from an HTML string, and makes it easier to write views without hyperscript or JSX. It accepts a tagged HTML string, meaning you can do interpolation like JSX with the [`${expression}` syntax](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Template_literals).
